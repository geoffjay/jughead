// provider.go is a vendored copy of services/github/provider.go, adjusted to
// reference sdk/auth (the canonical contract the plugin and host share)
// instead of sites/auth. The OAuth web-flow logic is identical to the
// in-repo provider so the plugin is a drop-in replacement.
package main
...
// auth.Register. It implements sdkauth.Provider; Load reads the GitHub OAuth
// credentials from the environment and produces a githubInstance.
func NewProvider() sdkauth.Provider { return githubProvider{} }
...
func (githubProvider) Load(cfg sdkauth.AuthConfig) (sdkauth.ProviderInstance, error) {
