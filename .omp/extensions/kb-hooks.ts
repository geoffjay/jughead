/**
 * jughead knowledge-base hooks for oh-my-pi.
 *
...
const TRIGGERS: ReadonlyArray<{ pattern: RegExp; hint: string }> = [
  {
    ...
    hint: "schema change → record a decision + concepts/data-model.md",
  },
  {
    pattern: /^db\/(rls|models)\.go$/,
    hint: "data model / RLS → concepts/data-model.md",
  },
  {
    pattern: /^sdk\//,
    hint: "public plugin API changed → record a decision (breaking-change surface)",
  },
  {
    pattern: /^plugin\/loader/,
    hint: "plugin loading → concepts/plugin-architecture.md",
  },
  {
    pattern: /^(middleware\/auth\.go|services\/github\/)/,
    hint: "auth flow → concepts/auth.md + decisions",
  },
  { pattern: /^cmd\/jughead\//, hint: "CLI surface → concepts/cli.md" },
  { pattern: /^templates\/init\//, hint: "scaffold templates → patterns/" },
];

const KB_INJECT_PREAMBLE =
  "# jughead knowledge base (injected)\n\n" +
