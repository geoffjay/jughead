...
TRIGGERS: list[tuple[re.Pattern, str]] = [
    (re.compile(r"^db/migrations/"), "schema change → record a decision + concepts/data-model.md"),
    (re.compile(r"^db/(rls|models)\.go$"), "data model / RLS → concepts/data-model.md"),
    (re.compile(r"^sdk/"), "public plugin API changed → record a decision (breaking-change surface)"),
    (re.compile(r"^plugin/loader"), "plugin loading → concepts/plugin-architecture.md"),
    (re.compile(r"^(middleware/auth\.go|services/github/)"), "auth flow → concepts/auth.md + decisions"),
    (re.compile(r"^cmd/jughead/"), "CLI surface → concepts/cli.md"),
    (re.compile(r"^templates/init/"), "scaffold templates → patterns/"),
]
...
def state_dir(session_id: str) -> str:
    d = os.path.join(tempfile.gettempdir(), "jughead-kb-state", session_id or "unknown")
